...
namespace FoodZone.Web.Helpers
{
    public class BackgroundJobFunction
    {
        ...
        public void UpdateReservation()
        {
            ...
            {
                var command = "Update Reservations Set Status = -1, CancelReason = N'Khách không đến',UpdatedAt = GETDATE() WHERE DATEADD(minute, 5, ReservationDate) <= GETDATE() and Status = 0";
        ...
        public void UpdateTable()
        {
            using (var ctx = new FoodZoneContext())
            {
                var ids = ctx.Database.SqlQuery<int>("Select t.Id from Tables as t Right Join ReservationDetails as rd on t.Id = rd.TableId right join Reservations as r on rd.ReservationId = r.Id Where DATEADD(minute, 5, r.ReservationDate) <= GETDATE() and t.Status = 1 and r.Status = 0").ToList();
                if (ids.Count() > 0)
                {
        ...
        public void ResetAllTable()
        {
            ...
            {
                resetTime = DateTime.Parse(getToday).AddHours(14).ToString("yyyy-MM-dd HH:mm:ss");
            }
            else
            {
                resetTime = DateTime.Parse(getToday).AddHours(21).ToString("yyyy-MM-dd HH:mm:ss");
            }
            using (var ctx = new FoodZoneContext())
            {
